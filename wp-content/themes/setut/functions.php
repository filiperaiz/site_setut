...
function more_text($text, $size, $symbol=''){
    $t = strlen(strip_tags($text));
    $str = '';
    if($t>$size){
        $str = substr(strip_tags($text), 0, $size);
        if(empty($symbol)){
            $str.='[...]';
        }else{
            $str.=$symbol;
...
{
    // Add Bootstrap default CSS
    wp_enqueue_style('sparkling-custom', get_template_directory_uri() . '/assets/css/custom.css');
